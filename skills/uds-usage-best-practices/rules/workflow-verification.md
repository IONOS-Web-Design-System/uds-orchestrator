...
2. **Typography** — variant, weight, color all match (use exact `Text` props)
   - If the app is running locally, use **Playwright MCP** `browser_evaluate` with
     `getComputedStyle()` to extract `fontSize`, `fontWeight`, `lineHeight`,
     `fontFamily` and compare against the Figma design's typography specs (see
     "Playwright MCP Verification — Typography" below).
3. **SUPER CRITICAL: Spacing pixel accuracy** — audit EVERY padding, margin,
   and gap value:
   ...
   - If the app is running locally, use **Playwright MCP** as the final check:
     ...
     compare px values against Figma (see "Playwright MCP Verification — Spacing"
     below). If Playwright MCP is not configured, suggest the user set it up.
...
7. **Colors** — only UDS tokens; no hardcoded hex values
   - If the app is running locally, use **Playwright MCP** `browser_evaluate` with
     `getComputedStyle()` to extract `color`, `backgroundColor`, `borderColor` and
     compare the resolved RGB/RGBA values against the Figma design's color values
     (see "Playwright MCP Verification — Colors" below).
8. **SUPER CRITICAL: Asset handling by category** — verify every non-text asset
   ...
   - **Raster images** (photos, PNGs, JPGs): uses `AspectRatio`, not raw
     `<img>` tags; `ratio` matches Figma frame; container width matches;
     `objectFit` matches Figma fill/fit behavior
   ...
   - If the app is running locally, use **Playwright MCP** `browser_evaluate` with
     `getBoundingClientRect()` to verify rendered asset dimensions (width/height)
     and aspect ratios match the Figma design (see "Playwright MCP Verification —
     Asset Dimensions" below).
...
4. **Compare** — the returned values are in **px** (browsers resolve rem -> px
   ...
   - Any mismatch indicates a wrong token mapping — refer back to the tables
     in `rules/core-spacing-tokens.md`
...
```
-> Padding is `space/4` (24px) -- Tailwind class should be `p-4`
-> Gap is `space/3` (16px) -- Tailwind class should be `gap-3`
...
4. **Compare** — browsers return colors in `rgb()` or `rgba()` format.
   To compare against Figma hex values:
   - `rgb(0, 0, 0)` = `#000000`
...
4. **Compare** — browsers return resolved values:
   - `fontSize` is in **px** — compare directly against Figma type specs
     (e.g., `"16px"` for body text, `"24px"` for headings)
...
4. **Compare** — the returned pixel dimensions should match the Figma frame:
   ...
   - `aspectRatio` should match the Figma frame's width/height ratio (e.g.,
     a 16:9 image -> `1.778`, a 4:3 image -> `1.333`, a square -> `1.0`)

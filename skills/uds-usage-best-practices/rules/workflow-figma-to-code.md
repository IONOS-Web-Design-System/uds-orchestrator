...
Complete reference for translating Figma designs to UDS React code, including asset
handling and custom component creation. Referenced from SKILL.md section 10.
...
- Remember tokens are 3D (brand x platform x color-scheme):
  ...
  - Gap tokens (`--gap-*`): use with `gap` alias — `p-gap-4`, `gap-gap-8`
    (for small decorative gaps)
...
Asset from Figma design
  ...
  +- NOT in the UDS icon package
      ...
      +- Is it raster (PNG/JPG/photo)? ---> Category 1: Raster Asset (AspectRatio)
```

### Category 1: Raster Assets (photos, screenshots, PNGs, JPGs)

**Use the `AspectRatio` component — never raw `<img>` tags for raster images.**

1. **Download the asset** from Figma and store in `public/` or `assets/`
2. **Use `AspectRatio`** with the correct `ratio` prop matching the Figma
   frame's width:height proportion
3. **Control the container width** via a parent wrapper to match Figma dimensions
4. **Verify after implementation** — aspect ratio, container width, and
   object-fit must match the Figma design exactly

```tsx
import AspectRatio from '@ionos-web-design-system/react/aspect-ratio';

{/* Match the Figma frame's aspect ratio and width */}
<div className="w-full max-w-[320px]">
  <AspectRatio src="/product-photo.jpg" ratio="4/3" alt="Product" />
</div>
```

See `rules/react-aspect-ratio.md` for the full ratio reference and
...
the UDS icon library. Follow the Figma MCP's download suggestions for the
asset URL, then:
...
<div className="w-[240px] h-[160px]">
  <img src="/illustration.svg" className="h-full w-full" alt="Feature illustration" />
</div>
```
...
component for brand logos.**

The `Icon` component applies fixed icon sizing that distorts logo proportions.
Brand logos need the exact width/height from the Figma design, controlled via
a `<div>` wrapper.
...
- **Always use a `<div>` wrapper** to control size — set Figma dimensions on
  the wrapper, not the `<img>` directly
- The `<img>` fills its container with `h-full w-full`
- Choose the Light/Dark variant matching the active color scheme
  (e.g., `ionosLight` for light mode, `ionosDark` for dark mode)
...
- Use the `Price` component for ALL pricing, including discounts,
  strike-through prices, and postlines
...
function ProductCard({ title, price, badge }) {
  return (
    <Surface
      variant="base"
      className="rounded-(--protected-container-rounded) p-4 flex flex-col gap-3"
    >

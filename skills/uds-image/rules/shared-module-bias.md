...
| `testimonial_slider` | **photoreal**, image-type **avatar** | **small, square 512×512** | One person per slide, square crop. Set the image type to **avatar** and frame it as that type requires: one person who still reads as the subject at thumbnail size, with no face-visibility requirement added here — a visible face is the brief's call, not this module's. Optionally a **companion asset** accompanies the avatar in the testimonial card; when *this* brief is the companion, compose it as a small, clean **supporting asset** that sits beside the avatar: a product as a `cutout`, or a `device-focused`/`scene` photoreal vignette (illustrations are produced by agent-svc, not here). |

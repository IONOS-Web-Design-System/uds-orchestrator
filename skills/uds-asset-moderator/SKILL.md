...
Hybrid plans MUST include an `embedStyle` field with one of five values. In the background styles the generated image is a backdrop the illustration's UI floats over; in `floating-card` the image is a contained card with UI chips at its edges; in `interface-asset` the image lives inside the rendered interface as its content; in `inline-asset` the image is a plain, contained media/hero asset inside an interface, panel or card, with no floating chips of its own. The interface is never composited into a pictured device's display.

- `background-pointer` — the image becomes a large rounded backdrop card over a brand-gradient base, cropped/zoomed to its relevant region; the illustration marks a calm negative-space region of the imagery with a design-tool selection marquee (no rendered marketing headline — see `rules/shared-brief-parsing.md` no-marketing-heading), plus a floating feature panel connected to it by a pointer line. Choose when the feature acts on the user's content shown in the imagery. **LARGE canvases only** (width ≥ 800 AND height ≥ 450) — the marquee + panel + connector need the room. The moderator demotes this style on smaller canvases, so plan accordingly: on a small canvas choose `floating-card`, `background-full`, or `interface-asset` directly instead.
...
- `floating-card` — any canvas size; the **small-format-native** counterpart to `background-pointer` (never demoted). The image is a single contained rounded card (dominant, ~60-80% of canvas) on a pale brand canvas — NOT full-bleed — with 1-2 small highlight chips/pills (a labelled chip and/or an icon pill) overlapping the card's edges; no marquee, no connector lines. Choose when the subject is the hero shown as a compact card and a couple of UI accents annotate it — the typical look for square / small-banner / inline-badge canvases.
- `inline-asset` — any canvas size. The illustration owns the canvas and paints its own background; the image is placed as a plain, contained media/hero asset **inside** a specific interface, panel or card slot the brief describes — never full-bleed, never the canvas backdrop, and never a dominant highlight card in its own right (that's `floating-card`'s job). **Catalog first**: prefer an existing catalog asset for the slot (mode `illustration` + `assets`, no image leg); choose `embedStyle: "inline-asset"` only when no offered catalog asset fits and the slot genuinely needs bespoke imagery.

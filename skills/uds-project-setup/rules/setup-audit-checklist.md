...
Structured checklist for auditing an existing project's UDS configuration. Run
through each section to identify and fix configuration issues.

## 1. Package Installation

- [ ] npm is configured with JFrog registry auth for the
...
**Check for Vite:** Read `index.html` **Check for Next.js:** Read
`app/layout.tsx` — look for `data-*` on `<html>`

## 4. ThemeProvider

- [ ] `ThemeProvider` imported from
      `@ionos-web-design-system/react/theme-provider`
...
**Check:** Grep for `ThemeProvider` in the entry component (`App.tsx` or
`layout.tsx`).
...
**Check:** `grep -rn '#[0-9a-fA-F]\{3,8\}' src/` and
`grep -rn 'rgb\(|rgba\(' src/`

## 7. Border Convention (Tailwind v4)

- [ ] All UDS border colors use double-dash: `border--base`,
      `border--semantic-*`
...
- [ ] `Text` component used for all styled text (not raw `<h1>`, `<p>` with
      Tailwind)
...
- [ ] Icon inject functions passed as references (`icon={plus}` not
      `icon={plus()}`)

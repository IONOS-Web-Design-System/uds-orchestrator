...
(typically `index.css` or `globals.css`) is the single source for all CSS imports.

## Golden Rule

**Never import UDS CSS in JS/TS files.** All `@import` statements go in the CSS entry file.
...
5. Shop-UI styles (if used) — `@ionos-web-design-system/shop-ui/style.css`
...
**Warning:** Never use wildcards in production — they include all 8 brands and
2 platforms, significantly increasing bundle size.

## With Shop-UI

Shop-UI styles come AFTER React component styles:
...
In Next.js, the CSS entry file is typically `app/globals.css` or `styles/globals.css`:
...
Do NOT put `@import 'tailwindcss'` in `vite.config.ts` — it belongs in the CSS file.
